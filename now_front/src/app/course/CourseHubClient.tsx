  ...
  return (
    <>
      {/* h1은 반드시 Suspense 바깥에 둔다 — 안쪽 콘텐츠는 useSearchParams() 때문에 클라이언트
          전용으로 렌더링돼 서버 HTML에는 fallback(빈 div)만 담긴다. 그래서 h1을 안에 넣으면
          크롤러 눈에는 h1이 아예 없는 페이지가 된다(2026-09-11 빙 웹마스터도구 지적).
          lang은 클라이언트에서만 알 수 있으므로 canonical 기준인 한국어로 고정. */}
      <h1 className="sr-only">서울 3시간 AI 추천 코스</h1>
