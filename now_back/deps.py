...
# 서울시 실시간 도시데이터(혼잡도) API 지점명 매핑 — 프론트/라우터가 쓰는 짧은 키(홍대/성수/강남역/이태원) ↔
...
#  '강남역'은 그대로 정확. '이태원'은 오류이고 '이태원 관광특구'가 정확한 등록명.
#  강북은 넓은 지역이라 이태원 외 지점(광화문 등)으로 향후 확장 예정 — 프론트 CROWD_REGION_POINTS도 같이 갱신할 것).
CROWD_AREA_MAP = {
    '홍대': '홍대 관광특구',
    '성수': '성수카페거리',
    '강남역': '강남역',
