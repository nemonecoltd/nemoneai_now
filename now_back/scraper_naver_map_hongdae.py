...
# [중요] Geocoding을 위한 API KEY (Gemini와 동일한 것 사용 가능)
...
async def scrape_naver_map_popups_hongdae():
    """홍대 지역 팝업스토어 정보 스크래핑 및 Gemini 정제"""
    results = []
    async with async_playwright() as p:
        ...
        naver_url = f"https://search.naver.com/search.naver?query=홍대+팝업스토어+공연+버스킹+일정&where=news&pd=3&ds={ds}&de={de}"
        print(f"🌐 홍대 네이버 데이터 발굴 중 (최근 3개월): {naver_url}")
        await page.goto(naver_url)
        await page.wait_for_timeout(2000)
        raw_text = "[Naver News Search Result]\n" + await page.inner_text("body")

        # 2. 구글 뉴스 검색 추가 (최근 3개월 제한)
        google_news_url = "https://news.google.com/search?q=홍대+팝업스토어+공연+버스킹+일정+when:3m&hl=ko&gl=KR&ceid=KR%3Ako"
        print(f"🌐 홍대 구글 뉴스 데이터 발굴 중 (최근 3개월): {google_news_url}")
        try:
            await page.goto(google_news_url, timeout=30000)
            await page.wait_for_timeout(2000)
            raw_text += "\n\n[Google News Search Result]\n" + await page.inner_text("body")
        except Exception as e:
            print(f"⚠️ 홍대 구글 뉴스 검색 실패: {e}")
        
        await browser.close()
        
        if len(raw_text.strip()) < 200:
            print("⚠️ 홍대 텍스트 확보 실패")
            return []
        ...
        prompt = f"""
        아래 지저분한 텍스트 뭉치에서 '홍대/서교동/연남동' 지역의 [팝업스토어, 인디 공연, 버스킹, 라이브 클럽, 전시] 정보만 찾아내줘.
        ...
        *명령:
        ...
        print("🧠 Gemini가 홍대 장소 정보를 정제 중입니다...")
        ...
        try:
            ...
            for item in clean_data:
                ...
                if not addr or ("마포구" not in addr and "홍대" not in addr):
                    addr = f"서울 마포구 홍대 {item['title']}"

                lat, lng = get_lat_lng(addr)
                results.append({
                    "naver_place_id": f"raw_hongdae_{hash(item['title']) % 10000}",
                    "title": item['title'],
            ...
            print(f"❌ 홍대 데이터 정제 실패: {e}")

    return results

if __name__ == "__main__":
    res = asyncio.run(scrape_naver_map_popups_hongdae())
    print(f"🚀 홍대 최종 {len(res)}개 수집 성공")
