"""서울시 실시간 도시데이터(혼잡도) 조회 — 지도 탭 카드/메인 티커용.
scraper_seoul_crowd.py(main.py APScheduler, 10분 간격)가 채워둔 crowd_status를 읽기만 한다."""
...
_ROW_COLUMNS = """
    area_nm, congest_lvl, prev_congest_lvl, ppltn_min, ppltn_max,
...
@router.get("/crowd/all")
async def get_crowd_all():
    """전체 지점 한 번에 반환 — 메인 티커(split-flap)가 지점별로 개별 호출하지 않도록.
    CROWD_AREA_MAP 순서를 그대로 따라 프론트가 순회 순서를 신경 안 써도 되게 정렬."""
    with engine.connect() as conn:
        rows = {
            row["area_nm"]: row
            for row in conn.execute(text(f"SELECT {_ROW_COLUMNS} FROM crowd_status")).mappings()
        }
    return [_serialize(rows[area]) for area in CROWD_AREA_MAP if area in rows]
...
async def get_crowd(area: str):
    ...
    with engine.connect() as conn:
        row = conn.execute(
            text(f"SELECT {_ROW_COLUMNS} FROM crowd_status WHERE area_nm = :area"),
            {"area": area},
        ).mappings().first()
